    ...
    return (
        history.length > 0 || loading === true ? (<div className='w-full h-full overflow-auto'>
          <InfiniteScroll
            ...
            endMessage={<span className='flex justify-center items-center p-8 text-neutral font-montserrat'>No more history</span>}
          >
                    ...
                    >
                      ...
                      <td className='px-1 flex justify-center items-center md:gap-x-2 grow gap-y-1 w-auto'>
                        <div className=" flex justify-start items-center gap-x-2 md:gap-x-10 w-full">
                          {x?.match?.Player1?.username ?  <div className="flex justify-center items-center text-xs font-poppins font-medium w-20 md:w-28 text-neutral">{x.match.Player1.username}</div> : <Load/>}
                          {x?.match ?  <Link to={`/Profile/${x.match.Player1.id}`}><img className='md:rounded-2xl rounded-xl h-8 md:h-12' src={x.match.Player1.avatar.medium} alt="Avatar" /> </Link> : <Load/>}                
                          <div className='flex flex-row items-center  gap-x-1 justify-center w-16  md:w-20 h-6 rounded-r-3xl rounded-l-3xl bg-neutral text-accent font-poppins'> 
                            <span className='font-poppins font-medium'>{x.match.Player1.score}</span><span className='font-poppins font-medium'> : </span><span className='font-poppins font-medium'>{x.match.Player2.score}</span>
                          </div>
                          
                          {x?.match?.Player2?.id ? <Link to={`/Profile/${x.match.Player2.id}`}><img className='md:rounded-2xl rounded-xl h-8 md:h-12' src={x.match.Player2.avatar.medium } alt="Avatar Tailwind CSS Component" />  </Link> : <Loading props={"lg"}/>}
                          <div className="flex justify-center items-center text-xs font-poppins font-medium  w-20 md:w-28 text-neutral">{x?.match?.Player2?.username  ? x.match.Player2.username : <Loading props={"sm"}/>}  </div> 
                        </div>
                      </td>
                      <td className='flex px-1 grow-0 justify-end items-center gap-x-1  w-auto'><div className={`w-18 ${getColor(x.match.Player1.score,x.match.Player2.score)}`}> {x.match.Player1.score > x.match.Player2.score && "+1"} {x.match.Player1.score < x.match.Player2.score && "-1"}{x.match.Player1.score === x.match.Player2.score && "0"}</div></td>
                    </tr>
                  ))}
                  { loading && (
                      <Loading size={"lg"}/>
